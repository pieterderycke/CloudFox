...
namespace CloudFox.Weave
{
    public class WeaveProxy
    {
        ...
        public WeaveProxy(ICommunicationChannel communicationChannel, string userName,
        ...
        public WeaveProxy(ICommunicationChannel communicationChannel, string userName,
            string password, string passphrase, string userServerUrl)
        {
        ...
        public IEnumerable<T> DecryptPayload<T>(IEnumerable<WeaveBasicObject> enumerable)
            where T : class
        {
        ...
        public T DecryptPayload<T>(WeaveBasicObject weaveBasicObject)
            where T : class
        {
        ...
        private void DownloadGlobalMetaData()
        {
            ...
            {
                // If we cannot find the global meta data it means that a desktop Firefox version must re-sync first.
                throw new ServerOutOfSyncException("The data on the server is out of sync, please first synchronize " +
                    "a desktop version of Firefox with the server.", ex);
            }
        }
        ...
        private T DecryptCipherText<T>(byte[] symetricKey, byte[] iv, string cipherString)
            where T : class
        {
